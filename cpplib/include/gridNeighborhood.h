...
class moGridSolNeighborhood : public moRndNeighborhood<moGridSolNeighbor<d>>
{
   public:
      ...
      moGridSolNeighborhood() : _grid() { }

      moGridSolNeighborhood(const Grid<d> grid) : _grid(grid) { }

      virtual bool hasNeighbor(EOT&) { return true; } //There is always an available neighbor

      virtual void init(EOT &solution, Neighbor &current);

      virtual void next(EOT &solution, Neighbor &current);

      virtual bool cont(EOT&) { return true; }
...
template <size_t d>
void moGridSolNeighborhood<d>::next(EOT &solution, Neighbor &current)
{ }
